...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		if sle.voucher_type == "Delivery Note":
			party = frappe.get_doc("Delivery Note",sle.voucher_no).customer
		elif sle.voucher_type == "Purchase Receipt":
			party = frappe.get_doc("Purchase Receipt",sle.voucher_no).supplier

		data.append([sle.date, sle.item_code, item_detail.item_name, item_detail.item_group,
			item_detail.brand, item_detail.description, sle.warehouse,party,
			item_detail.stock_uom, sle.actual_qty, sle.qty_after_transaction,
			(sle.incoming_rate if sle.actual_qty > 0 else 0.0),
			sle.valuation_rate, sle.stock_value, sle.voucher_type, sle.voucher_no, sle.company])

	return columns, data

def get_columns():
	return [_("Date") + ":Datetime:95", _("Item") + ":Link/Item:130", _("Item Name") + "::100", _("Item Group") + ":Link/Item Group:100",
		_("Brand") + ":Link/Brand:100", _("Description") + "::200", _("Warehouse") + ":Link/Warehouse:100","Party:Data:150",
