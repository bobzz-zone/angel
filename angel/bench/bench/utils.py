...
def init(path, apps_path=None, no_procfile=False, no_backups=False,
	...
	from .app import get_app, install_app, install_apps_from_path
	...
	if wheel_cache_dir:
        ...
        # customization is done to not use frappe code to revamp client server again
	#if not frappe_path:
	#	frappe_path = 'https://github.com/frappe/frappe.git'
	#get_app('frappe', frappe_path, branch=frappe_branch, bench=path, build_asset_files=False)
        dest_path = os.path.join(path, 'apps')
        src_path = os.path.join(os.path.join('/home', getpass.getuser()), 'frappe')
        shutil.move(src_path, dest_path)
        install_app('frappe', bench=path)
